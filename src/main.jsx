...
function App() {
  ...
  const deletePerson = () => { if (code !== '6767' || selectedId === 'arthur') return; setPeople((current) => { const next = { ...current }; const removed = next[selectedId]; delete next[selectedId]; Object.values(next).forEach((person) => { person.children = person.children.filter((id) => id !== selectedId); if (person.partnerId === selectedId) person.partnerId = removed?.partnerId || null }); return next }); setSelectedId('arthur'); setDeleteMode(false); setCode('') }
